...
define([
    ...
    $.widget('mage.shippingZoneSelectorModal', modal, {
        options: {
            ...
            getZoneName: function () {
                if (this.zone_name) {
                    return translate('Current shipping zone: ' +
                        this.zone_name);
                } else {
                    return translate('Empty shipping zone');
                }
            }
        },
        ...
        saveChanges: function () {
            ...
            $.ajax(this.options.save_url, {
                ...
                success: function (responseData) {
                    if (responseData.customer_data.zone_name) {
                        $('#shipping-zone-name').html(translate('Current shipping zone: ' +
                            responseData.customer_data.zone_name));
                    } else {
                        $('#shipping-zone-name').html(translate('Empty shipping zone'));
                    }
    ...
    return Component.extend({
        ...
        initialize: function () {
            ...
            if (this.isEmpty(customerData.get('location').country_code) || typeof cacheStorage["location"] == 'undefined') {
                new Promise(function (resolve, reject) {
                    resolve(customerData.reload(['location']));
                }).then(
                    function (result) {
            ...
            new Promise(function (resolve, reject) {
                var timer = setInterval(function () {
                    ...
                    if (customerData.get('location')().country_code) {
                        clearInterval(timer);
